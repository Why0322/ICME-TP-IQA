...
def get_dataloader(config, dataset_name, logger):
    ...
    if dataset_name == config.DATA.DATASET:
        ...
        dataset_path = os.path.join(base_path, "LiveChallenge")
        count = 1162
        batch_size = 16
        sel_num = list(range(0, count))
        random.shuffle(sel_num)
        ...
        logger.info(test_index)
        return build_dataloader(config, dataset_name, dataset_path, batch_size, test_index)
    elif dataset_name == "aigc1k":
        dataset_path = os.path.join(base_path, "AIGC-1k")
        count = 1080
        batch_size = 128
        sel_num = list(range(0, count))
        random.shuffle(sel_num)
...
def cross_eval(config, model, logger):
    ...
    if config.DATA.DATASET == "live":
        ...
        test_dataset = ["bid"]
    elif config.DATA.DATASET == "aigc1k":
        test_dataset = ["aigc1k", "aigc1k"]    
    result = []
    model.eval()
    with torch.no_grad():
        for idx, dataset in enumerate(test_dataset):
            ...
            temp_pred_scores = []
            temp_gt_scores = []
            for n_iter, (img, labels, _, _,_) in enumerate(val_loader):
