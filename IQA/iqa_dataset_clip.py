...
map2label = {'jpeg2000 compression':0, 'jpeg compression':1, 'noise':2, 'blur':3, 'color':4,
             'contrast':5, 'overexposure':6, 'underexposure':7, 'spatial':8, 'quantization':9, 'other':10,'others':10}

scene2label = {'animal':0, 'cityscape':1, 'human':2, 'indoor':3, 'landscape':4, 'night':5, 'plant':6, 'still_life':7,'still-life':7,
               'others':8,'other':8}

qualitylabel = {'bad':0, 'poor':1, 'fair':2, 'good':3, 'excellent':4}
...
class LIVECDATASET_clip(data.Dataset):
    def __init__(self, root, index, patch_num, transform=None):
        image_data = {}
        label_path = "/home/codes/TP-IQA/IQA/clive_all_clip_with_quality.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 7:
                    print(len(fields))
                    continue
                image_data[fields[0]] = (fields[2], fields[3],fields[6])
        ...
        for _, item in enumerate(index):
            for aug in range(patch_num):
                image_path = "Images/" + imgpath[item][0][0]
                # print("",image_path)
                dist, scene, quality = image_data.get(image_path)
                # if scene2 == "invalid":
                #     scene_label2 = -1
                # else:
                #     scene_label2 = scene2label[scene2]
                ...
                sample.append(
                    (os.path.join(root, "Images", imgpath[item][0][0]), labels[item],
                     scene2label[scene], map2label[dist_map[dist]],qualitylabel[quality])
                )
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (sample, target) where target is class_index of the target class.
        """
        path, target, scene, distortion, quality = self.samples[index]
        sample = self._load_image(path)
        sample = self.transform(sample)
        return sample, target, scene, distortion,quality
...
class LIVEDataset_clip(data.Dataset):
    def __init__(self, root, index, patch_num, transform=None):
        ...
        image_data = {}
        label_path = "/home/hongyu/codes/MP-IQE/IQA/live_all_clip_with_quality.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 7:
            ...
            for j, item in enumerate(train_sel):
                ...
                for aug in range(patch_num):
                    ...
                    try:
                        ...
                        # print(f"findpath:{find_path}")
                        dist, scene,quality= image_data.get(find_path)
                        sample.append(
                            (
...
class TID2013Dataset_clip(data.Dataset):
    ...
    def __init__(self, root, index, patch_num,transform=None):
        ...
        image_data = {}
        label_path = "/home/hongyu/codes/MP-IQE/IQA/tid2013_all_clip.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 5:
                ...
                # exit()
                image_data[fields[0]] = (fields[2], fields[3],fields[4])
        ...
        for i, item in enumerate(index):
            ...
            for j, item in enumerate(train_sel):
                ...
                # ref_dist = imgnames[item].split("_")[1]
                ...
                for aug in range(patch_num):
                    try:
                        image_path = os.path.join(root, "distorted_images", imgnames[item])
                        find_path = imgnames[item]
                        dist, scene,quality = image_data.get(find_path)
                        # print("",map2label[dist],scene2label[scene])
                        # exit()
                        sample.append(
                            (
                        ...
                        print("dist =", dist, "| scene =", scene, "| quality =", quality)
                        print("map2label 有:", list(map2label.keys()))
                        print("scene2label 有:", list(scene2label.keys()))
                        print("qualitylabel 有:", list(qualitylabel.keys()))
                        print("key error")
                        exit()
        self.samples = sample
        self.transform = transform
...
class CSIQDataset_clip(data.Dataset):
    def __init__(self, root, index, patch_num, transform=None):
        ...
        image_data = {}
        label_path = "/home/hongyu/codes/MP-IQE/IQA/csiq_all_clip_with_quality.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 7:
        ...
        for i, item in enumerate(index):
            ...
            train_sel = np.where(train_sel == True)
            train_sel = train_sel[0].tolist()
            # print(f"train_sel:{train_sel}")
            for j, item in enumerate(train_sel):
                label = transfer("csiq", labels[item])
                for aug in range(patch_num):
                    try:
                        image_path = os.path.join(root, "dst_imgs", imgnames[item])
                        find_path = image_path.split(root + '/')[-1]
                        dist, scene,quality = image_data.get(find_path)
                        sample.append(
                            (
...
class AIGCDATASET_clip(data.Dataset):
    def __init__(self, root, index, patch_num, transform=None):
        super(AIGCDATASET_clip, self).__init__()

 
        image_data = {}
        label_path = "/non_shared/codes/MP-IQE/IQA/aigc_all_clip_with_quality.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 5:
        ...
        xlsx_file = os.path.join(self.data_path, "AIGC_MOS_Zscore.xlsx")
        read = pd.read_excel(xlsx_file)
        imgname = read["Image"].values.tolist()
        ...
        for _, item in enumerate(index):
            for aug in range(patch_num):
                ...
                sample.append(
                    (
                        os.path.join(root,imgname[item]),
                        mos_all[item],
                        scene2label[scene], map2label[dist],qualitylabel[quality]
                    )
                )
...
class SPAQDATASET_clip(data.Dataset):
    def __init__(self, root, index, patch_num, transform=None):
        ...
        image_data = {}
        label_path = "/home/hongyu/codes/MP-IQE/IQA/spaq_all_clip_with_quality.csv"
        with open(label_path, "r") as f:
            for line in f:
                fields = line.strip().split('\t')
                if len(fields) != 7:
    ...
    def _load_image(self, path):
        try:
            ...
            #  # 获取原图尺寸并缩小一半
            w, h = im.size
            im = im.resize((w // 2, h // 2), Image.LANCZOS)   # 官方替代
            # exit()
